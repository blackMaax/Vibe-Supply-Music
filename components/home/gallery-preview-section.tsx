      ...
      <div className="container mx-auto px-4">
        ...
        <motion.div {...imageCardsContainerMotionProps}>
          ...
          {displayImages.map((img, index) => {
            // Determine image source (Sanity or static fallback)
            let imageUrl: string | null = null;
            if (img.asset && img.asset._ref) {
              try {
                imageUrl = urlForImage(img as any); // Cast as any if SanityImageReference type is complex
              } catch (e) {
                console.error("Error getting Sanity image URL for gallery:", e);
                imageUrl = getGalleryImage(index); // Fallback to static by index
              }
            } else if (img.src) {
              imageUrl = img.src;
            } else {
              imageUrl = getGalleryImage(index); // Fallback for placeholders or incomplete static data
            }
            
            if (!imageUrl) imageUrl = "/placeholder.svg"; // Final fallback
