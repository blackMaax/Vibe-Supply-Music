        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 bg-black/75 backdrop-blur-md border border-gold/30 rounded-2xl p-8 shadow-[inset_0_0_15px_rgba(0,0,0,0.3),0_10px_30px_rgba(0,0,0,0.15)] relative overflow-hidden">
          ...
          <div className="flex flex-col items-start relative z-10">
            ...
            <p className="text-sm text-gray-200 mb-8 max-w-xs">
              Elevating events with unforgettable live music experiences. Luxury entertainment for weddings and private functions.
            </p>
              ...
              {socialLinks && socialLinks.length > 0 ? (
                socialLinks.map((link) => link.url && (
                  <Link
                    ...
                    className="bg-white/10 w-10 h-10 rounded-full flex items-center justify-center hover:bg-gold hover:text-neutral-900 transition-colors duration-300 border border-gold/40"
                ...
                <>
                  <Link href="https://instagram.com" className="bg-white/10 w-10 h-10 rounded-full flex items-center justify-center hover:bg-gold hover:text-neutral-900 transition-colors duration-300 border border-gold/40" aria-label="Instagram"><Instagram size={18} /></Link>
                  <Link href="https://facebook.com" className="bg-white/10 w-10 h-10 rounded-full flex items-center justify-center hover:bg-gold hover:text-neutral-900 transition-colors duration-300 border border-gold/40" aria-label="Facebook"><Facebook size={18} /></Link>
                  <Link href="https://twitter.com" className="bg-white/10 w-10 h-10 rounded-full flex items-center justify-center hover:bg-gold hover:text-neutral-900 transition-colors duration-300 border border-gold/40" aria-label="Twitter"><Twitter size={18} /></Link>
                </>
              )}
            </div>
          </div>
            ...
            <ul className="space-y-4">
              ...
              <li className="flex items-center group">
                <div className="bg-white/10 w-10 h-10 rounded-full flex items-center justify-center mr-3 group-hover:bg-gold transition-colors duration-300 border border-gold/40">
                  <Phone size={18} className="text-gold group-hover:text-neutral-900 transition-colors duration-300" />
                </div>
                <div className="min-w-0">
                  <p className="text-sm text-gray-300">Phone</p>
                  <a href={`tel:${displayPhone}`} className="text-sm text-gray-200 hover:text-[var(--color-warm-gold)] transition-colors duration-300 break-words">{displayPhone}</a>
                </div>
              </li>
              )}
              {displayEmail && (
